...
+(void)registerInstance:(MyButton*)radioButton withGroupID:(NSString *)aGroupID{
...
+(void)buttonSelected:(MyButton*)radioButton{
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            MyButton *button = [rb_instances objectAtIndex:i];
...
-(void)defaultInit{
    ...
    // update follow:
    [MyButton registerInstance:self withGroupID:self.groupId];
